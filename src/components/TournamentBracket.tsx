...
export const TournamentBracket: React.FC<TournamentBracketProps> = ({
  ...
  const renderClubRow = (fixture: Fixture, side: 'home' | 'away') => {
    ...
    return (
      <div className={`flex items-center gap-2 rounded-xl px-2 py-1.5 transition sm:gap-2.5 sm:px-2.5 sm:py-2 ${winner ? 'bg-emerald-400/[0.08]' : 'bg-white/[0.018]'}`}>
        <div className="relative shrink-0">
          {tbd ? (
            <div className="flex h-6 w-6 items-center justify-center rounded-full border border-dashed border-white/[0.12] bg-white/[0.025] text-[8px] font-black text-slate-600 sm:h-7 sm:w-7 sm:text-[9px]">?</div>
          ) : (
            <ClubCrest
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-center gap-1">
            <span className={`truncate text-[11px] sm:text-xs ${winner ? 'font-black text-emerald-300' : tbd ? 'font-bold italic text-slate-500' : 'font-black text-slate-100'}`}>
              {tbd ? (customSourceLabel || sourceLabel(sourceFixtureId)) : `${seedPosition ? `#${seedPosition} ` : ''}${club?.name || clubId}`}
            </span>
            {isMe && <span className="shrink-0 rounded bg-amber-400/15 px-1 py-px text-[7px] font-black uppercase tracking-wide text-amber-300 sm:px-1.5 sm:py-0.5 sm:text-[8px]">Siz</span>}
          </div>
          {!tbd && (
            ownerInfo?.isClaimed && ownerInfo.userId ? (
              <button
                ...
                className="mt-0.5 block max-w-full truncate text-left text-[8px] font-medium text-slate-500 transition hover:text-blue-300 sm:text-[9px]"
              >
                {ownerInfo.displayText}
              </button>
            ) : (
              <div className="mt-0.5 truncate text-[8px] font-medium text-slate-600 sm:text-[9px]">{ownerInfo?.displayText || t.userNeeded}</div>
            )
          )}
        </div>

        <div className={`min-w-[28px] rounded-lg border px-1.5 py-0.5 text-center font-mono text-[13px] font-black sm:min-w-[30px] sm:px-2 sm:py-1 sm:text-sm ${winner ? 'border-emerald-400/25 bg-emerald-400/10 text-emerald-300' : 'border-white/[0.07] bg-black/20 text-slate-300'}`}>
  ...
  const renderMatchCard = (fixture: Fixture, round: RoundDefinition, index: number, featured = false) => {
    ...
    return (
      <button
        ...
        className={`group relative w-full overflow-hidden rounded-2xl border p-2 text-left shadow-xl transition duration-200 sm:p-2.5 ${featured ? 'min-h-[142px] border-amber-300/30 bg-gradient-to-br from-amber-500/[0.10] via-slate-950 to-slate-950 sm:min-h-[158px]' : 'border-white/[0.08] bg-[#0a111e]/95'} ${isMyMatch ? 'ring-1 ring-amber-300/45 shadow-amber-500/10' : ''} ${canOpen ? 'hover:-translate-y-0.5 hover:border-white/[0.18]' : ''}`}
      >
        {featured && <div className="pointer-events-none absolute right-0 top-0 h-16 w-16 rounded-bl-full bg-amber-400/[0.05] sm:h-20 sm:w-20" />}
        <div className="mb-1.5 flex items-center justify-between gap-2 px-0.5 sm:mb-2 sm:px-1">
          <div className="flex min-w-0 items-center gap-1.5">
            <span className={`text-[8px] font-black uppercase tracking-[0.14em] sm:text-[9px] sm:tracking-[0.16em] ${round.accent}`}>{round.shortLabel}</span>
            <span className="text-[8px] font-bold text-slate-600 sm:text-[9px]">M{index + 1}</span>
          </div>
          <span className={`flex shrink-0 items-center gap-1 rounded-full border px-1.5 py-0.5 text-[7px] font-black uppercase tracking-wide sm:px-2 sm:text-[8px] ${isProjected ? 'border-violet-400/20 bg-violet-400/[0.08] text-violet-300' : fixture.status === 'CONFIRMED' ? 'border-emerald-400/20 bg-emerald-400/[0.08] text-emerald-300' : fixture.status === 'DISPUTED' ? 'border-rose-400/20 bg-rose-400/[0.08] text-rose-300' : 'border-blue-400/20 bg-blue-400/[0.07] text-blue-300'}`}>
        ...
        {featured && (
          <div className="mt-2 flex items-center justify-center gap-1 text-[8px] font-black uppercase tracking-[0.16em] text-amber-300/70 sm:text-[9px] sm:tracking-[0.18em]">
  ...
  return (
    <section className="overflow-hidden rounded-[24px] border border-white/[0.08] bg-[linear-gradient(180deg,rgba(7,12,23,0.98),rgba(4,8,16,0.98))] shadow-2xl sm:rounded-[28px]">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex min-w-0 items-center gap-2">
              <h2 className="truncate text-lg font-black tracking-tight text-white sm:text-xl">{competition?.name || 'Domestic Cup'}</h2>
      ...
      {champion && (
        <div className="relative overflow-hidden border-b border-amber-300/20 bg-[radial-gradient(circle_at_center,rgba(251,191,36,0.13),transparent_55%)] px-4 py-5 text-center sm:px-5 sm:py-6">
          ...
          <div className="mx-auto flex h-11 w-11 items-center justify-center rounded-2xl border border-amber-300/25 bg-amber-400/[0.10] shadow-lg shadow-amber-500/10 sm:h-12 sm:w-12">
            <Crown className="h-5 w-5 text-amber-300 sm:h-6 sm:w-6" />
          </div>
          <div className="mt-3 text-[8px] font-black uppercase tracking-[0.23em] text-amber-300/70 sm:text-[9px] sm:tracking-[0.25em]">2026/27 Champion</div>
      ...
      {viewMode === 'bracket' && (
        <div className="relative overflow-x-auto overscroll-x-contain p-2.5 scrollbar-thin sm:p-4 lg:p-5">
          <div className="mb-2 flex items-center gap-1.5 px-1 text-[8px] font-bold uppercase tracking-wider text-slate-600 sm:hidden">
            <ChevronRight className="h-3 w-3 text-blue-300" />
            Swipe to follow the road to the final
          </div>
          <div className="flex min-w-max items-start gap-2.5 sm:gap-3.5 lg:gap-5">
            {rounds.map((round, roundIndex) => (
              <React.Fragment key={round.key}>
                <div className={`flex w-[246px] flex-col rounded-[20px] border p-2.5 sm:w-[255px] sm:p-3 lg:w-[270px] lg:rounded-[22px] ${round.glow}`}>
                  <div className="mb-2.5 flex items-center justify-between border-b border-white/[0.06] pb-2 lg:mb-3">
                    <div>
                      <div className={`text-[9px] font-black uppercase tracking-[0.16em] lg:text-[10px] lg:tracking-[0.18em] ${round.accent}`}>{round.label}</div>
                      <div className="mt-0.5 text-[8px] font-bold text-slate-600 lg:text-[9px]">{round.fixtures.length} matches</div>
                    </div>
                    <div className={`flex h-7 w-7 items-center justify-center rounded-xl border border-white/[0.07] bg-black/20 lg:h-8 lg:w-8 ${round.accent}`}>
                      {round.key === 'FINAL' ? <Trophy className="h-3.5 w-3.5 lg:h-4 lg:w-4" /> : round.key === 'SF' ? <Medal className="h-3.5 w-3.5 lg:h-4 lg:w-4" /> : round.key === 'PRELIM' ? <Flame className="h-3.5 w-3.5 lg:h-4 lg:w-4" /> : <GitBranch className="h-3.5 w-3.5 lg:h-4 lg:w-4" />}
                    </div>
                  </div>
                ...
                {roundIndex < rounds.length - 1 && (
                  <div className="flex h-20 w-5 shrink-0 self-center items-center justify-center sm:w-6 lg:h-24 lg:w-7">
                    <div className="relative h-full w-px bg-gradient-to-b from-transparent via-white/[0.12] to-transparent">
                      <div className="absolute left-1/2 top-1/2 flex h-6 w-6 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-white/[0.09] bg-[#080e19] text-slate-500 shadow-lg shadow-black/30 lg:h-7 lg:w-7">
