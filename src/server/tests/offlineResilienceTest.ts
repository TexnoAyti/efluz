...
async function main(): Promise<void> {
  ...
  const clubs = queryGet<any>(
    `SELECT id FROM clubs WHERE league_id = 'league-premier-league' ORDER BY id LIMIT 2`
  );
  if (!clubs) {
    throw new Error('Could not load baseline Premier League clubs from SQLite.');
  }

  const pair = queryGet<any>(
    `SELECT id, home_club_id, away_club_id
       FROM clubs c
       JOIN leagues l ON l.id = c.league_id
      WHERE l.id = 'league-premier-league'
      ORDER BY c.id
      LIMIT 1`
  );

  const clubRows = requireRows(
    `SELECT id FROM clubs WHERE league_id = 'league-premier-league' ORDER BY id LIMIT 2`
  );
  ...
  queryRun('DELETE FROM pending_mutations WHERE mutation_id = ?', [mutationId]);
...
function requireRows(sql: string): any[] {
  const { queryAll } = require('../db') as typeof import('../db');
  const rows = queryAll<any>(sql);
  if (rows.length < 2) throw new Error('Expected at least two Premier League clubs in SQLite baseline.');
  return rows;
}
